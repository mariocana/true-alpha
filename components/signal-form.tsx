...
export function SignalForm() {
  ...
  // Track if we're waiting for this specific transaction
  const txStartedRef = useRef(false)
  
  const { writeSignal, isPending, isConfirming, isConfirmed, error, hash } = useWriteSignal()
  ...
  // ONLY save if we have a pending signal AND the tx was started by us
  useEffect(() => {
    if (isConfirmed && pendingSignal && txStartedRef.current && !error) {
      console.log('✅ Transaction confirmed, saving signal to localStorage')
  ...
  }, [isConfirmed, pendingSignal, error])

  // Reset pending signal on error (user rejected or tx failed)
  useEffect(() => {
    if (error && pendingSignal) {
      console.log('❌ Transaction failed or rejected, not saving signal:', error.message)
      setPendingSignal(null)
      txStartedRef.current = false
    }
  }, [error, pendingSignal])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Write to blockchain - this triggers MetaMask
      const result = await writeSignal(signal)
      
      // Store pending signal to save AFTER confirmation
      if (result) {
        console.log('📝 Signal pending, waiting for on-chain confirmation...')
        setPendingSignal(result)
      }
    } catch (err) {
      console.error('Transaction rejected:', err)
      txStartedRef.current = false
      // Don't save anything if user rejects
    }
  }
    ...
    <Card className="border-neon-green/30">
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          {pendingSignal && isConfirming && (
            <div className="text-xs text-blue-400 border border-blue-400/30 p-2 rounded-sm">
